...
| 00012_audio_buffer_bucket.sql | ✅ applied | 2026-07-16 | `node apply-migrations.js supabase/migrations/00012_audio_buffer_bucket.sql` (direct pg, single txn; owner-approved EN-8 staging trial); verified live: `storage.buckets` id=tts-audio public=true, policy `tts_audio_public_read` on storage.objects, cron.job `tts-audio-orphan-backstop` schedule `0 3 * * *` | EN-8: public `tts-audio` buffer bucket + anon read/list RLS (writes/deletes service-role only) + pg_cron 7-day orphan backstop (copy-confirmed deletion is primary). Lands in the SHARED prod DB (COORD-2 BLOCKING-2, owner-accepted). Inert until write-back flag flips; staging Verpex pull cron installed 2026-07-16. Idempotent. |
| 00013_profiles_admin_select_update.sql | ✅ applied | 2026-07-15 | `node apply-migrations.js supabase/migrations/00012_profiles_admin_select_update.sql` (direct connection; applied live under the pre-renumber 00012 filename — renumbered to 00013 to avoid EN-8's 00012); verified live via anon client authed as admin — SELECT of another profile by email returns the row, UPDATE of another profile affects 1 row (both were 0/null before) | EN-15 RLS repair. Live profiles SELECT/UPDATE policies ("Users can view own profile" / "Users can update own profile") carried NO admin bypass (only `auth.uid() = id`) — contradicting the 00001 doc claim and blocking the EN-15 admin grant control. Recreate both with `OR public.is_admin()` (same helper/pattern as the live DELETE policy and 00009). Idempotent (DROP IF EXISTS). No column/data changes. |
