...
| 00012_audio_buffer_bucket.sql | ⏳ NOT applied (operator-gated) | — | pending: apply via Supabase Dashboard SQL editor after the Verpex feasibility spike is GO | EN-8: public `tts-audio` buffer bucket + anon read/list RLS (writes/deletes service-role only) + pg_cron 7-day orphan backstop (copy-confirmed deletion is primary). Lands in the SHARED prod DB (COORD-2 BLOCKING-2, owner-accepted). Do NOT apply until the write-back flag rollout is sequenced (both Verpex crons up first). Idempotent. |
