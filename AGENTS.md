...
- **Release notes (CHANGELOG) authoring (MANDATORY — granular + user-facing is the default):** the `CHANGELOG.md` top entry is the user's release notes, not a commit log. Write it for a non-technical reader:
  - **One bullet per distinct user-visible change — NOT one per ticket.** A ticket that ships four user-facing improvements gets four bullets. (This rule exists because 2026.07.19.1's EN-23b — four separate admin-audio fixes — was first collapsed into one bullet and the owner had to catch it by eye.)
  - **User-facing language only.** Describe what the user experiences. NO file/function/config names, internal IDs, code identifiers, or jargon in the prose. The trailing `(TB-1)`-style ticket tag is the *only* traceability marker allowed in a bullet.
  - **Lead verb by type, headline in bold:** `New:` / `Improved:` / `Fixed:` / `Security:` followed by a **bold headline** and a short plain-language explanation.
  - **Audience tag when not for everyone:** append `(admin)` (e.g. `Fixed (admin): …`) so end-user-only vs admin-only changes are distinguishable.
  - **Completeness:** every user-facing change since the last release has a bullet. Nothing shipped silently.
  - **Enforcement:** `scripts/check-changelog-completeness.mjs` runs in preflight and checks completeness (every shipped ticket has a bullet), flags over-collapsed entries (fewer bullets than shipped ticket-scopes), and flags technical tokens leaking into bullet prose. WARN-only during rollout (never blocks the build); promote to enforce with `CHANGELOG_GATE_ENFORCE=1` once tuned.
