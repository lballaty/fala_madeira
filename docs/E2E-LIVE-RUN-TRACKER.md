...
### Run 29 — 2026-07-20 — EF-39 fixed at the SOURCE (instant Quiz transition under fm:e2e); de-quarantined
- **EF-39 · `user/04-learning-feedback:107` — `verified` — source fix (`51a1190`):** the Run 26 idempotent choice-click retry mitigated but did **not** eliminate the flake (recurred; tolerated only by config `retries:1`). Root cause reconfirmed: choice buttons **and** the typed-answer `<input>` live inside the per-question `motion.div`, and `AnimatePresence mode="wait"` serializes an exit-then-enter window; under full-suite CPU load Playwright's stability check is defeated by janky animation frames, so a click/fill races the moving element and never registers → Next stays disabled → assertion times out. **Fix at the source (not another test band-aid):** gate the per-question transition to `{ duration: 0 }` behind the existing prod-safe test-only flag `localStorage['fm:e2e']==='1'` (same pattern as `VocabularyView`'s `E2E_ANSWER_HINT`) in `src/components/Quiz.tsx`; spec 04 sets the flag via `addInitScript` before first navigation. Instant transitions remove the moving-target window deterministically. **Zero product-behavior change** — real users keep the full animation (flag never set in the shipped app).
- **Verification:** isolated **3/3 first-attempt** (`--retries=0 --repeat-each=3`, fresh CI=1 build, 28.6s); full **preflight** all-green (11 stages); full **`CI=1 npm run test:e2e:regression`** from develop: **138 passed · 0 failed · 3 skipped of 141** (7.3m; 3 skips = owner-approved EN-8 `test.fixme`) — **EF-39 passed within the full-suite load run**.
- **Net:** EF-39's last open flake mechanism closed at the source. No longer relies on `retries:1` → **de-quarantined**. No release-version impact (develop feature commit; ship.sh owns version bumps at release).
...
- **`EF-39` · `user/04-learning-feedback:107` — `verified` — CLOSED at source (Run 29, 2026-07-20, `51a1190`) — owner: harness (flake):** ~~fails only under full-suite load~~ **RESOLVED.** The Run 26 idempotent choice-click retry mitigated but did not eliminate it (recurred; tolerated only by `retries:1`). Root cause: choice buttons + the typed `<input>` live inside the animated per-question `motion.div`; `AnimatePresence mode="wait"` serializes an exit-then-enter window that, under CPU load, defeats Playwright's stability check → click/fill races the moving element. **Fix (source, not test band-aid):** per-question transition `{ duration: 0 }` under the prod-safe test-only `fm:e2e` flag (`Quiz.tsx`); spec 04 sets the flag. Zero product-behavior change. Verified: isolated 3/3 first-attempt + full CI=1 regression 138/0/3. **De-quarantined** (no longer relies on `retries:1`); no longer blocks a fully-green CI gate.
