...
- **Drift-check false positives (documented, no action):** `settings.tutor.select` "João, 45" is runtime-composed (`{t.name}, {t.age}` — TutorSelectionModal.tsx:72; data intact in `src/data/tutors.ts`), so the static grep can't see it; "Dismiss hint" (ContextualHint) is DEAD UI — no live call site passes `onDismiss` (only the unit test exercises it). Remaining depth backlog: 9 rendered-only controls (incl. `admin.studio.save_draft/publish` — held at rendered deliberately: outcome-asserting them would publish to the live project).
