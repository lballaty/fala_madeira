...
- **EF-38 (NEW) · `admin/13-admin-audio-panel.spec.ts:31` "marks a clip bad and enqueues it for regeneration" — DETERMINISTIC (2/2 incl. targeted rerun) — owner: EN-23 (test-isolation / residual DB state) — `open`:** at spec line 62 the test expects `audio-enqueue` **enabled** after marking clip `tts:default:service:cb1b0ad1` bad, but it renders **`disabled` "Queued"** — a regeneration-queue row for that build-key **already persists in the live DB** from a prior run, so the app correctly shows it as already-queued. **Not a product bug** (the "Queued"/disabled state is correct for an already-enqueued clip) and **not this session's code** (EN-23 avoid-list: AdminView/admin-audio). Commit `a253591` ("make the e2e enqueue round-trip deterministic + self-isolating") does **not** reset this pre-existing queued row. **Next action (EN-23 build-side):** seed/clear the `tts:default:service:cb1b0ad1` regeneration-queue row in the spec's setup/teardown (or pick a per-run-unique build-key) so the enqueue path starts un-queued. Same seed/teardown-hygiene family as CS-8. Repro: `npx playwright test tests/e2e/admin/13-admin-audio-panel.spec.ts`.
