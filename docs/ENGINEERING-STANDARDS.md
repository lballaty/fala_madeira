...
- **Version bump before commit** when source files change: run the canonical `~/.ai-dev-dotfiles/tools/version-bump.py` (CalVer `YYYY.MM.DD.N`) in the commit-staging workflow — never via pre-commit hook. Docs-only changes don't bump.
- **Branch:** work on `main` (greenfield, owner directive). Deploy to Verpex from this device via `npm run deploy` (`scripts/ship.sh`) — GitHub is source hosting only, never a deploy path.
