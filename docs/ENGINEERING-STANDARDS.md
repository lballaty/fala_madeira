...
- **Preflight gate inventory (`scripts/preflight.sh` is canonical).** The pre-commit / pre-deploy chain runs these HARD stages in order: eslint (`lint:eslint`), tsc (`lint`), vitest (`test:run`), vite `build`, e2e interactive-coverage contract (`test:e2e:coverage`), npm audit (prod high/critical), `check-standards.sh`, CORS-header contract, help-drift contract (capability registry ↔ generated chat-help), IndexedDB `DB_VERSION` drift (app ↔ e2e helpers), and observability §9 forbidden-patterns (`--strict`) — plus changelog-completeness (WARN-only) and opt-in live security probes (`--with-security`). Keep this list in sync with `scripts/preflight.sh`; the script is the source of truth. The full Playwright regression suite (`npm run test:e2e` / `scripts/run-e2e-regression.mjs`, run with `CI=1`) is a separate live gate, not part of `preflight.sh`.
