...
### TB-8 — Daily voice/audio limit shows the default (5), not the configured global (20); config is hard to find; must persist (reporter: owner, staging 2026-07-14) — `OPEN (partially diagnosed)`
...
- **Fix direction:** (1) ensure the client always reflects the server value (await the global-settings load before enforcing/displaying, or seed initial state from the fetched value; drop the misleading localStorage-first default); (2) surface the limit read-only to all users (so they see their real allowance) and keep the editable control for admins but make it easier to find; (3) clarify global-vs-per-user in the label. Ties DF11 (persist + recall). Owner: Agent E/S. **Status:** OPEN — server value verified (20); client load path needs repro + fix.
