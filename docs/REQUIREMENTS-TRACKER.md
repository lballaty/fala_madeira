...
| LT10 | **supabase-js wedges after a page reload — queue replay stalls (offline variant) AND all REST traffic hangs (online variant)** — gotrue races its own session restoration on reloaded pages. Offline variant (EF-31): replay goes out unauthenticated (401) or hangs on gotrue's lock. **SCOPE BROADENED run 12 (EF-33):** a plain ONLINE reload can wedge the client so that ZERO REST requests leave the page (even the boot profile fetch) — user/34+35 wire-proven, reproducible ×2; app renders but is effectively read-only with infinite spinners. Sync-queue hardening shipped (78ddfd0); remaining fix: timeout-guarded `getSession()`→`refreshSession()` at the client/init seam (not only inside sync-queue) + retry ladder. Full evidence: E2E-LIVE-RUN-TRACKER EF-31 + EF-33. **ROOT CAUSE + FIX (024683b):** self-deadlock — `useAuth` awaited REST inside `onAuthStateChange`, which gotrue awaits inside its init lock; callback made synchronous + follow-up deferred to a macrotask; sync-queue drains timeout-guarded with a refreshSession fallback and 3s/10s/30s ladder. user/30+34+35 pass ×3 and in full suite (run 13, 72/75). Not yet deployed to prod. | e2e user/30, user/34, user/35 + Lane B probes/traces 2026-07-13 | ✅ fixed (verified; deploy pending) |
...
## Deferred / open follow-ups (2026-07-14 session)

Captured so nothing deferred verbally ("other agent's lane") gets buried. Every item has an owner + next action. Cross-refs: E2E-LIVE-RUN-TRACKER (EF-/PF-), plans/.plan-state-observability.yaml. Status legend as above (✅ done · 🔵 ready/in plan · 🟡 partial/in progress · ⬜ needs work · ❓ decision needed).

| ID | Item | Next action / notes | Status |
|---|---|---|---|
| DF1 | **Gamification: finish the "celebrate" layer** — restraint is deliberate (PRODUCT-DESIGN-TARGET §2/§10 "celebrate, never punish") but underbuilt: correct answers get only a static green highlight, `profiles.xp` exists but is **never awarded**, streak/level render as plain text. | Build shared `<CorrectPop>` micro-animation reused in Quiz/Listening/drills; award XP via sync-queue; subtle streak/level glow. Stay inside no-dark-patterns guardrails. | ⬜ |
| DF2 | **Translation: lesson patterns have no source data** — `Lesson.patterns: string[]` (types.ts:8) carries no English; tutor replies are dynamic. Deferred from the "hybrid" translation decision. | DECISION NEEDED: (A) bake translation into the data model (schema + gemini generate-lesson prompt + backfill — offline-capable) vs (B) on-demand translate on tap (reuses translate edge action; online-only, AI cost). | ❓ |
| DF3 | **Translation: tutor chat** — AI replies shown in PT with no reveal. Depends on DF2 approach (dynamic → on-demand translate). | Wire tap-to-reveal via translate edge action once DF2 direction chosen. | ⬜ |
| DF4 | **Translation: simulator FREE-mode NPC lines** — guided mode already shows `bubble.translation` (SimulatorView.tsx:244); free mode hides it with no peek. Data exists. `TranslatableText` primitive shipped (commit 1d16e6f). | Swap free-mode NPC bubble to `TranslatableText` (guided keeps always-on). Ready to build. | 🔵 |
| DF5 | **Translation discoverability** — existing affordances (listening "Reveal transcript" ListeningView.tsx:457; simulator guided toggle) may be under-discovered; part of the "everything's in Portuguese" perception may be UX prominence, not missing features. | Evaluate making existing reveals more visible before adding more reveal surfaces. | ❓ |
| DF6 | **Manual beta smoke on a FRESH free-tier account** — consent gate now persists post-PF-13 (defaults false → existing users re-affirm); verify TTS 503→device-speech fallback (deployed) + voice-cap upgrade prompt. | Owner to run one fresh-account pass before beta. | ⬜ |
| DF7 | **Release/worktree convention** — base repo checkout is shared by multiple agents, so branch switches conflict; releases (and any branch divergence) must use a dedicated git **worktree**, never a branch switch in the shared tree. `develop` = integration, `main` = release. | Document in AGENTS.md + reconcile stale ENGINEERING-STANDARDS §10 §147 ("work on main"). In progress this session. | 🟡 |
| DF8 | **Untracked `.aidevops/` + `.claude/version-history.json`** — sitting untracked; need commit-vs-gitignore decision. | Decide: `.claude/version-history.json` → gitignore (matches `.claude/settings.local.json`); `.aidevops/` → commit if in-repo scaffold else gitignore. | ❓ |
