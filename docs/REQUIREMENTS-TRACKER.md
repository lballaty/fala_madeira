...
| LT10 | **Reconnect replay stalls after an offline reload (narrow LT9 residual)** — gotrue races its own session restoration on pages reloaded while offline: replay goes out unauthenticated (401) or hangs on gotrue's lock. Sync-queue hardening shipped (78ddfd0); remaining fix: timeout-guarded `refreshSession()` on reconnect + retry ladder. Full evidence: E2E-LIVE-RUN-TRACKER EF-31. Only affects offline-reloaded pages; normal offline→reconnect drains fine (verified). | e2e user/30 + Lane B probes 2026-07-13 | ⬜ open |
