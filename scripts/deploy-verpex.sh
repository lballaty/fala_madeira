...
if command -v rsync >/dev/null 2>&1; then
  ...
  # EN-34 (symlink contention fix): the exclude is 'audio' WITHOUT a trailing slash. Once staging's
  # /audio is a SYMLINK to prod's /audio (single-puller model, §10/§11), a 'audio/' pattern (dir-only)
  # would NOT match the symlink, so `--delete` on a staging deploy would remove the link. The
  # slashless 'audio' matches the path segment whether it is a real dir or a symlink, keeping either
  # form intact across deploys. 'audio-sync/' (the cron dir) is unaffected — it is a distinct segment.
